...
"/RAYYANCALL"       { current_column+=yyleng; return CALL;}
{DIGIT}+            {yylval.strVal = strdup(yytext); current_column+=yyleng; return NUM;}
...
"/p"                {yylval.strVal = strdup(yytext); current_column+=yyleng; return PLUS;}

"/m"                {yylval.strVal = strdup(yytext); current_column+=yyleng; return MINUS;}

"/t"               {yylval.strVal = strdup(yytext); current_column+=yyleng; return TIMES;} 

"/d"               {yylval.strVal = strdup(yytext); current_column+=yyleng; return DIVIDE;}
...
"arrayOf_"    {return ARRAY_DATATYPE; current_column+=yyleng;} 
...
{IDENTIFIER}            {yylval.strVal = strdup(yytext); current_column+=yyleng; return IDENTIFIER;}
...
{DIGIT}+{IDENTIFIER} {
    fprintf(stderr, "Error at line %d, column %d: Identifier \"%s\" must not begin with a number\n", current_line, current_column, yytext);
    current_column += yyleng;
}

. {
    fprintf(stderr, "Unrecognized symbol at line %d, column %d: %s\n", current_line, current_column, yytext);
    current_column += yyleng;
    return VALID_CHAR;
}
